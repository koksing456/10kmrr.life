...
ensure_tracker() {
  if [[ ! -s "$TRACKER_DIR/pro-interest.csv" ]]; then
    printf 'Pro-interest tracker missing. Run ./script/alpha.sh tracker first.\n' >&2
    exit 1
  fi
}
...
self_test() {
  ...
  if "$0" --tracker-dir "$temp_dir/tracker" --tester-id tester_003 --notes 'ARR 120000' >/dev/null 2>&1; then
    printf 'record_alpha_pro_followup self-test failed: ARR-labelled money amount was accepted.\n' >&2
    exit 1
  fi
